...
import com.noistop.noistop.dtos.EstadoReporteCountDTO;
...
public class ReporteController {
    ...
    @Operation(summary = "Conteo de reportes por estado", description = "Retorna la cantidad de reportes agrupados por estado (pendiente, en revisión, resuelto, etc.).")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Conteo de reportes por estado"),
            @ApiResponse(responseCode = "404", description = "No existen reportes registrados")
    })
    @GetMapping("/conteo-por-estado")
    @PreAuthorize("hasAnyAuthority('ADMIN','AUTHORITY')")
    public ResponseEntity<List<EstadoReporteCountDTO>> conteoPorEstado() {
